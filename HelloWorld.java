...
public class HelloWorld {
    public static void main(String[] args) {
        ...
        int month = (int) Math.ceil(inputIndex / 4d);
        System.out.println(month);

        // 计算输入的周是这个月的第几周
        int startIndex = (month - 1) * 4 > 0 ? (month - 1) * 4 : 0;
        // 提示信息
        ...
        for (int i = 0; i < 4; i++) {
            String target = ((i + startIndex) == inputIndex - 1) ? "√" : "";
            System.out.println(target + contentList[i + startIndex]);
